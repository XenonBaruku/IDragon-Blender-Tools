...
def loadANM(filePath, armature = None, setFakeUser = False, framerate = 0):
    ...
    frameEnd = int(parsedANMData.duration * actual_framerate) - 1
    ...
    armatureObject.location = parsedANMData.armatureTranslation
    #armatureObject.rotation_euler = parsedANMData.armatureRotationEuler
    armatureObject.keyframe_insert(data_path="location", frame=0)
    armatureObject.keyframe_insert(data_path="location", frame=frameEnd)
    #armatureObject.keyframe_insert(data_path="rotation_euler", frame=0)
    ...
    for poseBone in armatureObject.pose.bones:
        ...
        poseBone.keyframe_insert(data_path="scale", frame=0)
    ...
    for poseBone in armatureObject.pose.bones:
        poseBone.keyframe_insert(data_path="location", frame=frameEnd)
        poseBone.keyframe_insert(data_path="rotation_quaternion", frame=frameEnd)
        poseBone.keyframe_insert(data_path="scale", frame=frameEnd)
